...
import type { NewWipTodo } from './types'

type AddWipTodoDialogProps = {
  projectName: string
  onAddTodo: (todo: NewWipTodo) => Promise<void>
}

const buttonStyles =
  'inline-flex min-h-10 cursor-pointer items-center justify-center gap-2 rounded-lg px-3 text-sm font-bold transition-colors data-[focus-visible]:outline-3 data-[focus-visible]:outline-offset-2 data-[focus-visible]:outline-focus disabled:cursor-not-allowed disabled:opacity-50'

export function AddWipTodoDialog({
  projectName,
  onAddTodo,
}: AddWipTodoDialogProps) {
  ...
  return (
    <DialogTrigger>
      <Button
        className={`${buttonStyles} mt-4 w-full bg-accent text-white data-[hovered]:bg-accent-hover`}
      >
            ...
            return (
              <>
                ...
                <p className="mt-2 mb-6 text-sm leading-6 text-muted">
                  Capture an unassigned draft todo for {projectName}.
                </p>
                ...
                >
                  ...
                  >
                    ...
                    <Input
                      autoFocus
                      placeholder="Summarize the draft todo"
